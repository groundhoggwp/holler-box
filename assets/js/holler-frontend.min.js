...
}};b.noteListeners=function(f){d("body").on("click",".hwp-close",b.hideItem).on("click","#hwp-floating-btn",b.btnClick).on("click",".hwp-interaction",b.interactionLink).on("click",".hwp-full-side",b.fullSide).on("click",".hwp-text i",b.sendText).on("click","#hwp-"+f+" .hwp-email-btn",b.emailSubmitClick);
...
var j=0.5;if((h/(i-g))>j&&!b.show["hwp-"+f]){b.showNote(f);b.show["hwp-"+f]=true;}},250));};b.fullSide=function(g){g.stopImmediatePropagation();var h=d(g.target).closest(".holler-box").attr("id");
...
},1000);return false;};b.showNote=function(h){var f=c.hollerVars[h];var g=a.getElementById("hwp-"+h);d("#hwp-"+h+" .hwp-first-row").html(f.content);if(f.hideFirst==="true"||b.getCookie("hwp-"+h+"_hide")==="true"){b.transitionOut(g);
if(f.hideBtn!="1"&&f.position!="holler-banner"){b.transitionIn(b.floatingBtn);}}else{b.transitionIn(g);if(f.hideBtn!="1"&&f.position!="holler-banner"){b.transitionOut(b.floatingBtn);
}}if(f.showEmail==="1"&&f.showChat!="1"){b.showEmailSubmit(h);}if(f.hideBtn==="1"){b.hide(b.floatingBtn);}if(f.showChat==="1"){b.transitionIn(a.querySelector("#hwp-"+h+" .hwp-chat"));
}if(f.position==="holler-banner"&&b.getCookie("hwp-"+h+"_hide")!="true"){b.toggleBnrMargin(h);}if(f.hide_after==="delay"){setTimeout(function(){b.transitionOut(g);
...
}else{var f=d("#hwp-"+h).outerHeight();d("body").css("margin-top",f);}};b.hideItem=function(g){g.stopImmediatePropagation();var f=d(g.target).closest(".holler-box");
var h=f.attr("id").split("-")[1];b.toggleHide(h);if(f.hasClass("holler-banner")){b.toggleBnrMargin(h,true);}return false;};b.btnClick=function(f){f.stopImmediatePropagation();
...
}else{b.setCookie("hwp-"+g+"_hide","true",1);setTimeout(function(){b.clearChat(g);},1000);}b.showNote(g);};b.transitionIn=function(f){f.style.display="block";
...
},100);};b.transitionOut=function(f){d(f).addClass("hwp-transition-out").removeClass("hwp-show");setTimeout(function(){d(f).removeClass("hwp-transition-out").addClass("hwp-hide");
...
}var h=a.querySelector("#hwp-"+i+" .hwp-email-input");h.setAttribute("placeholder",g.placeholder);d(".hwp-away-msg").remove();d("#hwp-"+i+" .hwp-email-row").prepend('<span class="hwp-away-msg">'+g.optinMsg+"</span>");
d("#hwp-"+i).addClass("has-optin");f.removeClass("hwp-hide").addClass("hwp-show");f.prepend('<span class="hwp-away-msg">'+g.optinMsg+"</span>");};b.emailSubmitClick=function(f){f.stopImmediatePropagation();
...
}if(c.hollerVars[h].emailProvider==="ck"){b.ckSubscribe(f,h);return;}else{if(c.hollerVars[h].emailProvider==="mc"){b.mcSubscribe(h);return;}}var g=c.localStorage.getItem("hwp-full-msg");
...
d.ajax({method:"POST",url:h,data:{email:g,api_key:f.ckApi}}).done(function(k){console.log(k);b.showConfirmation(j);d("#hwp-"+j+" .hwp-email-row").hide();
b.conversion(j);}).fail(function(k){d("#hwp-"+j+" .hwp-email-row").prepend('<span id="hwp-err">There seems to be a problem, can you try again?</span>');
setTimeout(function(){d("#hwp-err").remove();},3000);console.log(k);});};b.mcSubscribe=function(g){var f=d("#hwp-"+g+" form.hwp-mc-form");d.ajax({type:"GET",url:f.attr("action"),data:f.serialize(),cache:false,dataType:"jsonp",jsonp:"c",contentType:"application/json; charset=utf-8",error:function(h){console.log(h);
...
}d("#hwp-"+g+" .hwp-email-row").prepend('<span id="hwp-err">'+h+"</span>");setTimeout(function(){d("#hwp-err").remove();},3000);}else{b.showConfirmation(g);
d("#hwp-"+g+" .hwp-email-row").hide();b.conversion(g);}}});};b.sendMsg=function(f,g,h){d.ajax({method:"GET",url:c.hollerVars.ajaxurl,data:{email:f,id:h,msg:g,action:"hwp_send_email",nonce:c.hollerVars.hwpNonce}}).done(function(i){console.log(i);
...
var g=d(this).data("id");b.conversion(g);};b.conversion=function(g){var f={action:"hwp_track_event",nonce:c.hollerVars.hwpNonce,id:g};d.ajax({method:"GET",url:c.hollerVars.ajaxurl,data:f}).done(function(h){console.log(h);
}).fail(function(h){console.log(h);});b.setCookie("hwp_"+g+"_int","true",1);};b.handleForms=function(f){var g=d(f.target).closest(".holler-box").attr("id").split("-")[1];
setTimeout(function(){b.conversion(g);d(f.target).closest(".hwp-email-row").hide();b.showConfirmation(g);},1000);};b.showConfirmation=function(h){var f=c.hollerVars[h];
