...
}},g*1000);}else{b.detectScroll(k);}};b.noteListeners=function(f){d("body").on("click",".hwp-close",b.hideItem).on("click","#hwp-floating-btn",b.btnClick).on("click","#hwp-"+f+" a",b.interactionLink).on("click",".hwp-text i",b.sendText).on("click","#hwp-"+f+" .hwp-email-btn",b.emailSubmitClick);
...
var j=0.5;if((h/(i-g))>j&&!b.show["hwp-"+f]){b.showNote(f);b.show["hwp-"+f]=true;if(b.getCookie("hwp-"+f+"_hide")!="true"){b.countNoteShown(f);}}},250));
};b.showNote=function(h){var f=c.hollerVars[h];var g=a.getElementById("hwp-"+h);if(b.getCookie("hwp-"+h+"_hide")==="true"){b.transitionOut(g);if(f.hideBtn!="1"&&f.position!="holler-banner"){b.transitionIn(b.floatingBtn);
}}else{b.transitionIn(g);d("#hwp-"+h+" .hwp-first-row").html(f.content);if(f.hideBtn!="1"&&f.position!="holler-banner"){b.transitionOut(b.floatingBtn);
...
},parseInt(f.hide_after_delay)*1000);}};b.setCookie=function(g,k,h,i){var j=new Date();j.setTime(j.getTime()+(h*24*60*60*1000));var f="expires="+j.toUTCString();
if(!i){i="/";}a.cookie=g+"="+k+";"+f+";path="+i;return k;};b.getCookie=function(h){var g=h+"=";var k=decodeURIComponent(a.cookie);var f=k.split(";");for(var j=0;
j<f.length;j++){var l=f[j];while(l.charAt(0)==" "){l=l.substring(1);}if(l.indexOf(g)==0){return l.substring(g.length,l.length);}}return"";};b.debounce=function(g,f){var h=null;
...
},1);setTimeout(function(){d(f).removeClass("hwp-transition-in").addClass("hwp-show");},300);};b.transitionOut=function(f){d(f).addClass("hwp-transition-out").removeClass("hwp-show");
...
}b.showEmailSubmit();}};b.submitChatTextOnEnter=function(f){if(f.target.value&&f.target.value!=""&&f.keyCode==13){var g=d(f.target).closest(".holler-box").attr("id").split("-")[1];
...
}else{d("#hwp-"+h+" .hwp-first-row").html(g);}if(f.showChat==="1"){b.clearChat(h);}};b.countNoteShown=function(i){var f=c.hollerVars[i];var g=(f.hideForDays?parseInt(f.hideForDays):1);
b.setCookie("hwp_"+i+"_shown","true",g);var h={action:"hwp_track_view",nonce:c.hollerVars.hwpNonce,id:i};d.ajax({method:"GET",url:c.hollerVars.ajaxurl,data:h}).done(function(j){}).fail(function(j){});
