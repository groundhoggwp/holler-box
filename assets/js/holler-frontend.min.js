...
d.ajax({method:"GET",url:c.hollerVars.ajaxurl,data:f}).done(function(h){console.log(h);}).fail(function(h){console.log(h);});b.setCookie("hwp_"+g+"_int","true",1);
};b.handleForms=function(f){var g=d(f.target).closest(".holler-box").attr("id").split("-")[1];setTimeout(function(){b.conversion(g);d(f.target).closest(".hwp-email-row").hide();
b.showConfirmation(g);},1000);};b.showConfirmation=function(h){var f=c.hollerVars[h];var g=(f.confirmMsg!=""?f.confirmMsg:"Thanks!");if(f.position==="holler-banner"){d("#hwp-"+h+" .hwp-box-rows").addClass("hwp-full-width").html(g);
}else{d("#hwp-"+h+" .hwp-first-row").html(g);}if(f.showChat==="1"){b.clearChat(h);}};b.countNoteShown=function(g){var f={action:"hwp_track_view",nonce:c.hollerVars.hwpNonce,id:g};
d.ajax({method:"GET",url:c.hollerVars.ajaxurl,data:f}).done(function(h){console.log(h);}).fail(function(h){console.log(h);});};b.init();c.hollerbox=b;})(window,document,jQuery);
