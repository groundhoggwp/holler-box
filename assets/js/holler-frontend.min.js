...
},1000);return false;};b.showNote=function(h){var f=c.hollerVars[h];var g=a.getElementById("hwp-"+h);if(b.getCookie("hwp-"+h+"_hide")==="true"){b.transitionOut(g);
if(f.hideBtn!="1"&&f.position!="holler-banner"){b.transitionIn(b.floatingBtn);}}else{b.transitionIn(g);d("#hwp-"+h+" .hwp-first-row").html(f.content);if(f.hideBtn!="1"&&f.position!="holler-banner"){b.transitionOut(b.floatingBtn);
...
var g=c.hollerVars[i];if(f.hasClass("hwp-show")){return;}var h=a.querySelector("#hwp-"+i+" .hwp-email-input");h.setAttribute("placeholder",g.placeholder);
...
};b.emailSubmitClick=function(f){f.stopImmediatePropagation();f.preventDefault();var g=d(f.target).closest(".holler-box").attr("id").split("-")[1];b.emailSubmitted(g);
...
return;}if(d("#hwp-"+h+" input[name=hwp_hp]").val()!=""){return;}if(c.hollerVars[h].emailProvider==="ck"){b.ckSubscribe(f,h);return;}else{if(c.hollerVars[h].emailProvider==="mc"){b.mcSubscribe(h);
...
d.ajax({method:"GET",url:c.hollerVars.ajaxurl,data:f}).done(function(h){}).fail(function(h){});};b.init();c.hollerbox=b;})(window,document,jQuery);