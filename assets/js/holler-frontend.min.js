...
return;}if(d("#hwp-"+h+" input[name=hwp_hp]").val()!=""){return;}if(c.hollerVars[h].emailProvider==="ck"){b.ckSubscribe(f,h);return;}else{if(c.hollerVars[h].emailProvider==="mc"){b.mcSubscribe(f,h);
...
setTimeout(function(){d("#hwp-err").remove();},3000);console.log(k);});};b.mcSubscribe=function(g,h){var f=d("#hwp-"+h+" .mc-list-id").val();d.ajax({method:"GET",url:c.hollerVars.ajaxurl,data:{email:g,list_id:f,action:"hwp_mc_subscribe",nonce:c.hollerVars.hwpNonce}}).done(function(i){console.log(i);
b.showConfirmation(h);d("#hwp-"+h+" .hwp-email-row").hide();b.conversion(h);}).fail(function(i){console.log(i);});};b.sendMsg=function(f,g,h){d.ajax({method:"GET",url:c.hollerVars.ajaxurl,data:{email:f,id:h,msg:g,action:"hwp_send_email",nonce:c.hollerVars.hwpNonce}}).done(function(i){console.log(i);
