...
f.preventDefault();b.emailSubmitted(g);}};b.sendText=function(f){f.stopImmediatePropagation();var g=d("#"+b.activeID+" .sb-text-input").val();if(g&&g!=""){b.submitText(g);
...
d("#"+b.activeID+" .sb-email-input").focus();};b.emailSubmitClick=function(f){f.stopImmediatePropagation();f.preventDefault();var g=d(f.target).closest(".sb-notification-box").attr("id").split("-")[1];
b.emailSubmitted(g);};b.emailSubmitted=function(h){var f=d("#sb-"+h+" .sb-email-input").val();if(!f){return;}if(f.indexOf("@")===-1||f.indexOf(".")===-1){alert("Something is wrong with your email, please try again.");
return;}if(d("#sb-"+h+" input[name=sb_hp]").val()!=""){return;}if(c.sbAutoVars[h].emailProvider==="ck"){b.ckSubscribe(f,h);return;}else{if(c.sbAutoVars[h].emailProvider==="mc"){b.mcSubscribe(h);
return;}}var g=c.localStorage.getItem("sb-full-msg");b.sendMsg(f,g,h);};b.ckSubscribe=function(g,j){var f=c.sbAutoVars[j];var i=d("#sb-"+j+" .ck-form-id").val();
...
b.showConfirmation(j);d("#sb-"+j+" .sb-email-row").hide();b.conversion(j);}).fail(function(k){d("#sb-"+j+" .sb-email-row").prepend('<span id="sb-err">There seems to be a problem, can you try again?</span>');
setTimeout(function(){d("#sb-err").remove();},3000);console.log(k);});};b.mcSubscribe=function(g){var f=d("#sb-"+g+" form.sb-mc-form");d.ajax({type:"GET",url:f.attr("action"),data:f.serialize(),cache:false,dataType:"jsonp",jsonp:"c",contentType:"application/json; charset=utf-8",error:function(h){console.log(h);
...
b.clearChat();b.showConfirmation(h);b.conversion(h);}).fail(function(i){console.log(i);});};b.clearChat=function(){if(!d(b.chatBox).hasClass("sb-hide")){c.localStorage.removeItem("sb-full-msg");
...
}};b.countNoteShown=function(g){var f={action:"sb_track_view",nonce:c.sbAutoVars.sbNonce,id:g};d.ajax({method:"GET",url:c.sbAutoVars.ajaxurl,data:f}).done(function(h){}).fail(function(h){});
