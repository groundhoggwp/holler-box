...
}}if(j.showSettings==="interacts"&&b.getCookie("sb_"+k+"_int")!=""){return;}b.activeID="sb-"+k;b.activeOptions=j;b.cacheSelectors();b.noteListeners();if(j.bgColor){h.style.backgroundColor=j.bgColor;
...
f.toggleClass("sb-minimizing",!f.hasClass("sb-full-side"));setTimeout(function(){f.removeClass("sb-minimizing");},30000);return false;};b.showNote=function(){var f=b.activeOptions;
...
if(f.hideBtn!="1"){b.transitionIn(b.floatingBtn);}}else{b.transitionIn(h);if(f.hideBtn!="1"){b.transitionOut(b.floatingBtn);}}if(f.showEmail==="1"){b.transitionIn(b.noteOptin);
var g=a.querySelector("#"+b.activeID+" .sb-email-input");if(!g){return;}g.setAttribute("placeholder",f.placeholder);d(".sb-away-msg").remove();d("#"+b.activeID+" .sb-email-row").prepend('<span class="sb-away-msg">'+b.activeOptions.optinMsg+"</span>");
}if(f.hideBtn==="1"){b.hide(b.floatingBtn);}if(f.hide_after==="delay"){setTimeout(function(){b.transitionOut(h);},parseInt(f.hide_after_delay)*1000);}};
...
}if(l.indexOf(g)==0){return l.substring(g.length,l.length);}}return"";};b.debounce=function(g,f){var h=null;return function(){var j=this,i=arguments;clearTimeout(h);
h=setTimeout(function(){g.apply(j,i);},f);};};b.hideItem=function(f){b.toggleHide();return false;};b.toggleHide=function(){var f=c.localStorage.getItem("sb_hide_note");
...
}b.showNote();};b.transitionIn=function(f){f.style.display="block";setTimeout(function(){d(f).addClass("sb-transition-in").removeClass("sb-hide");},1);
setTimeout(function(){d(f).removeClass("sb-transition-in").addClass("sb-show");},100);};b.transitionOut=function(f){d(f).addClass("sb-transition-out").removeClass("sb-show");
