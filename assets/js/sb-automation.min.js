...
}};b.doNotification=function(){b.noteVisitorChecks();};b.doBanner=function(){};b.noteVisitorChecks=function(){if(b.getCookie("sb_note_int")==="true"){b.noteInteracted=true;
b.currentContent=(c.sbAutoVars.noteInteracted?c.sbAutoVars.noteInteracted:c.sbAutoVars.noteDefault);}else{if(b.getCookie("sb_visit")===""){var f=b.setCookie("sb_visit",Date.now(),parseInt(c.sbAutoVars.expires));
...
}}setTimeout(function(){b.showNote(b.currentContent);b.noteListeners();},b.delay);};b.noteListeners=function(){d("body").on("click",".sb-close",b.hideItem).on("click","#sb-submit-email",b.emailSubmitted).on("click","#sb-floating-btn",b.toggleHide).on("click",".sb-interaction",b.interacted).on("click","#sb-submit-text",b.sendText);
...
}}else{b.transitionIn(b.notificationBox);if(f.showBtn!="false"){b.transitionOut(b.floatingBtn);}}if(f.showOptin==="true"){b.transitionIn(b.noteOptin);var g=a.getElementById("sb-text-input");
if(g){g.setAttribute("placeholder",f.placeholder);}}if(f.showBtn==="false"){b..hide(b.floatingBtn);}if(f.showChat==="true"){b.transitionIn(b.chatBox);}};
...
},7000);};b.showEmailSubmit=function(){var f=d(".sb-email-row");if(f.hasClass("sb-show")){return;}f.removeClass("sb-hide").addClass("sb-show");f.prepend('<span class="sb-away-msg">'+c.sbAutoVars.optinMsg+"</span>");
...
b.clearChat();b.showConfirmation();b.interacted(null);}).fail(function(h){console.log(h);});};b.clearChat=function(){if(!d(b.chatBox).hasClass("sb-hide")){c.localStorage.removeItem("sb-full-msg");
d(".sb-visitor-row, .sb-away-msg").remove();}b.hide(b.noteOptin);};b.interacted=function(){d.ajax({method:"GET",url:c.sbAutoVars.ajaxurl,data:{action:"sb_track_event",nonce:c.sbAutoVars.sbNonce}}).done(function(f){console.log(f);
}).fail(function(f){console.log(f);});b.setCookie("sb_note_int","true",1);};b.showConfirmation=function(){b.firstRow.innerHTML=b.currentContent.confirmMsg;
