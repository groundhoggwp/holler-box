...
}if(j.display_when!="scroll"){var f=(j.display_when==="delay"?parseInt(j.delay):0);setTimeout(function(){b.showNote(k);},f*1000);}else{b.detectScroll(k);
...
var h="https://aapi.convertkit.com/v3/forms/"+i+"/subscribe";d.ajax({method:"POST",url:h,data:{email:g,api_key:f.ckApi}}).done(function(k){console.log(k);
b.showConfirmation(j);d("#sb-"+j+" .sb-email-row").hide();b.interacted(j);}).fail(function(k){d("#sb-"+j+" .sb-email-row").prepend('<span id="sb-err">There seems to be a problem, can you try again?</span>');
...
d("#sb-"+g+" .sb-email-row").hide();b.interacted(g);}}});};b.sendMsg=function(f,g,h){d.ajax({method:"GET",url:c.sbAutoVars.ajaxurl,data:{email:f,id:h,msg:g,action:"sb_send_email",nonce:c.sbAutoVars.sbNonce}}).done(function(i){console.log(i);
