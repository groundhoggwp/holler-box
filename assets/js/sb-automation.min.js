...
}};b.doActive=function(g){for(var f=0;f<g.length;f++){b.doChecks(g[f]);}};b.doChecks=function(j){var h=a.getElementById("sb-"+j);if(!h){return;}var i=c.sbAutoVars[j];
if(!i.visitor){return;}if(i.visitor==="new"&&b.newVisitor!=true){return;}if(i.visitor==="returning"&&b.newVisitor!=false){return;}var g=b.getCookie("sb_"+j+"_shown");
if(i.show_settings==="once"&&g==="true"){return;}else{if(i.show_settings==="once"&&g===""){b.setCookie("sb_"+j+"_shown","true",1);}}b.activeID="sb-"+j;
b.activeOptions=i;b.cacheSelectors();b.noteListeners();if(i.display_when!="scroll"){var f=(i.display_when==="delay"?parseInt(i.delay):0);setTimeout(function(){b.showNote();
},f*1000);}else{b.detectScroll();}};b.checkCookie=function(){if(b.getCookie("sb_visit")===""){b.setCookie("sb_visit",Date.now(),parseInt(c.sbAutoVars.expires));
...
}if(f.showBtn==="false"){b.hide(b.floatingBtn);}if(f.showChat==="1"){b.transitionIn(b.chatBox);}if(f.hide_after==="delay"){setTimeout(function(){b.transitionOut(h);
