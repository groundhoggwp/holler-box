...
}}if(j.showSettings==="interacts"&&b.getCookie("sb_"+k+"_int")!=""){return;}b.floatingBtn=a.getElementById("sb-floating-btn");b.noteListeners();if(j.bgColor){h.style.backgroundColor=j.bgColor;
...
b.setCookie("sb_new","true",1);b.newVisitor=true;}else{if(b.getCookie("sb_new")!=""){b.newVisitor=true;}}};b.noteListeners=function(){d("body").on("click",".sb-close",b.hideItem).on("click",".sb-email-btn",b.emailSubmitted).on("click","#sb-floating-btn",b.btnClick).on("click",".sb-interaction",b.interactionLink).on("click",".sb-full-side",b.fullSide).on("click",".sb-text i",b.sendText).on("submit",".sb-notification-box form",b.handleForms);
d(".sb-text-input").on("keypress",b.submitChatTextOnEnter);d(".sb-email-input").on("keypress",b.submitEmailOnEnter);};b.detectScroll=function(f){d(c).scroll(b.debounce(function(){var h=d(c).scrollTop(),i=d(a).height(),g=d(c).height();
...
}if(f.position==="sb-banner-top"&&b.getCookie("sb-"+i+"_hide")!="true"){b.toggleBannerClass();}if(f.hide_after==="delay"){setTimeout(function(){b.transitionOut(h);
...
d("body").toggleClass("has-sb-banner-top");};b.hideItem=function(g){g.stopImmediatePropagation();var f=d(g.target).closest(".sb-notification-box");var h=f.attr("id").split("-")[1];
b.toggleHide(h);if(f.hasClass("sb-banner-top")){b.toggleBannerClass();}return false;};b.btnClick=function(f){f.stopImmediatePropagation();var g=d(this).data("id");
b.toggleHide(g);};b.toggleHide=function(g){var f=b.getCookie("sb-"+g+"_hide");if(f==="true"){b.setCookie("sb-"+g+"_hide","true",-1);}else{b.setCookie("sb-"+g+"_hide","true",1);
setTimeout(function(){b.clearChat();},1000);}b.showNote(g);};b.transitionIn=function(f){console.log("in",f);f.style.display="block";setTimeout(function(){d(f).addClass("sb-transition-in").removeClass("sb-hide");
},1);setTimeout(function(){d(f).removeClass("sb-transition-in").addClass("sb-show");},100);};b.transitionOut=function(f){console.log("out",f);d(f).addClass("sb-transition-out").removeClass("sb-show");
...
d("#"+b.activeID+" .sb-visitor-row, #"+b.activeID+" .sb-away-msg").remove();}b.hide(b.noteOptin);};b.interactionLink=function(f){var g=d(this).data("id");
b.interacted(g);};b.interacted=function(g){var f={action:"sb_track_event",nonce:c.sbAutoVars.sbNonce,id:g};d.ajax({method:"GET",url:c.sbAutoVars.ajaxurl,data:f}).done(function(h){console.log(h);
...
var g=(f.confirmMsg!=""?f.confirmMsg:"Thanks!");if(f.position==="sb-banner-top"){d("#sb-"+h+" .sb-box-rows").addClass("sb-full-width").html(g);}else{b.firstRow.innerHTML=g;
