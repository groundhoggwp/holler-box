...
};b.detectScroll=function(f){d(c).scroll(b.debounce(function(){var h=d(c).scrollTop(),i=d(a).height(),g=d(c).height();var j=0.5;if((h/(i-g))>j&&!b.show["sb-"+f]){b.showNote(f);
b.show["sb-"+f]=true;}},250));};b.fullSide=function(g){g.stopImmediatePropagation();var h=d(g.target).closest(".sb-notification-box").attr("id");var f=d("#"+h);
f.toggleClass("sb-full-side");f.toggleClass("sb-minimizing",!f.hasClass("sb-full-side"));setTimeout(function(){f.removeClass("sb-minimizing");},1000);return false;
};b.showNote=function(i){var f=c.sbAutoVars[i];var h=a.getElementById("sb-"+i);d("#sb-"+i+" .sb-first-row").html(f.content);if(f.hideFirst==="true"||b.getCookie("sb-"+i+"_hide")==="true"){b.transitionOut(h);
...
}else{b.setCookie("sb-"+g+"_hide","true",1);setTimeout(function(){b.clearChat();},1000);}b.showNote(g);};b.transitionIn=function(f){console.log("in",f);
f.style.display="block";setTimeout(function(){d(f).addClass("sb-transition-in").removeClass("sb-hide");},1);setTimeout(function(){d(f).removeClass("sb-transition-in").addClass("sb-show");
},100);};b.transitionOut=function(f){console.log("out",f);d(f).addClass("sb-transition-out").removeClass("sb-show");setTimeout(function(){d(f).removeClass("sb-transition-out").addClass("sb-hide");
...
f.target.value="";}};b.submitEmailOnEnter=function(f){if(f.target.value&&f.target.value!=""&&f.keyCode==13){b.emailSubmitted();}};b.sendText=function(f){f.stopImmediatePropagation();
var g=d("#"+b.activeID+" .sb-text-input").val();if(g&&g!=""){b.submitText(g);d("#"+b.activeID+" .sb-text-input").val("").focus();}};b.submitText=function(g){var f=c.localStorage.getItem("sb-full-msg");
...
};b.emailSubmitClick=function(f){f.stopImmediatePropagation();var g=d(f.target).closest(".sb-notification-box").attr("id").split("-")[1];console.log(g);
...
return;}var g=c.localStorage.getItem("sb-full-msg");b.sendMsg(f,g,h);};b.sendMsg=function(f,g,h){d.ajax({method:"GET",url:c.sbAutoVars.ajaxurl,data:{email:f,id:h,msg:g,action:"sb_send_email",nonce:c.sbAutoVars.sbNonce}}).done(function(i){console.log(i);
